...
pub fn query(
    table_input: HashMap<String, TableInput>,
    ...
    let table_columns = HashMap::from([
        ...
        ("customer".into(), vec!["c_name", "c_custkey", "c_acctbal"]),
        (
            "lineitem".into(),
    ...
    let customer_csvreader_node = build_csv_reader_node("customer".into(), &table_input, &table_columns);
    let orders_csvreader_node = build_csv_reader_node("orders".into(), &table_input, &table_columns);
    let lineitem_csvreader_node = build_csv_reader_node("lineitem".into(), &table_input, &table_columns);
    ...
    // HASH JOIN Node
    let co_hash_join_node = HashJoinBuilder::new()
    ...
    co_hash_join_node.subscribe_to_node(&customer_csvreader_node, 0); // Left Node
    co_hash_join_node.subscribe_to_node(&where_node, 1); // Right Node
    lo_hash_join_node.subscribe_to_node(&lineitem_csvreader_node, 0); // Left Node
    lo_hash_join_node.subscribe_to_node(&co_hash_join_node, 1); // Right Node
